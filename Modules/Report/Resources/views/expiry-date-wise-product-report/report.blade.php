                        ...
                        <td class="text-center">
                            @if (config('settings.logo'))
                                <a href="{{ url('dashboard') }}">
                                    <img src="{{ asset('storage/'.LOGO_PATH.config('settings.logo'))}}" style="max-width: 200px;margin-bottom:10px;" alt="Logo" />
                                </a>
                            @else
                            ...
                            font-weight: bold;background: black;border-radius: 10px;width: 320px;color: white;text-align: center;padding:5px 0;}">EXPIRY DATE WISE PRODUCT REPORT</p>
...
{{--                            <div class="text-center"><div style="font-weight: normal;margin:0;font-weight:bold;">{{ $product->name }}</div></div>--}}
                ...
                <table cellspacing="0" cellpadding="0" id="product_table">
                    ...
                    <tr>
                        ...
                        <td class="text-center font-weight-bolder">INV No.</td>
                        <td class="text-center font-weight-bolder">Product</td>
                        <td class="text-center font-weight-bolder">Unit</td>
                        <td class="text-center font-weight-bolder">Exp. Date</td>
                    ...
                    @if (!$report_data->isEmpty())
                        @foreach ($report_data as $key => $value)
                            <tr>
                                ...
                                <td class="text-center"> {{ $value->purchase->invoice_no ?? '' }} </td>
                                <td class="text-center"> {{ $value->product->name }} - {{ $value->product_variant->item_code }} </td>
                                <td class="text-center"> {{ $value->unit->unit_name }} </td>
                                <td class="text-center"> {{ date('d-m-Y',strtotime($value->expiry_date)) }} </td>
                                <td class="text-center"> {{ number_format($value->total_qty,2,'.',',') }} </td>
                            </tr>
                        @endforeach
                    @else
                        <tr><td colspan="5" class="text-center" style="color: red;font-weight:bold;">No Data Found</td></tr>
                    @endif
                    </tbody>
                </table>
            </div>
