                        ...
                        <form action="" id="sale_return_form" method="post" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class="form-group col-md-4 required">
                                    <label for="memo_no">{{__('Invoice No')}}.</label>
                                    ...
                                    <div class="form-group">
                                        <div class="">
                                            <table class="table table-bordered quarterSelectTable" id="saleTable">
                                                ...
                                                <th width="13%" >{{__('Warehouse')}}</th>
                                                <th width="12%" >{{__('Product')}}</th>
                                                <th width="5%" >{{__('Unit')}}</th>
                                                <th width="10%" >{{__('Price')}}</th>
                                                <th width="10%" >{{__('Sale Qty')}}</th>
                                                <th width="10%" >{{__('Returned Qty')}}</th>
                                                <th width="10%" >{{__('Return Qty')}}</th>
...
{{--                                                    @endphp--}}
                                                    <tr class="text-center">
                                                        ...
                                                        <td><input type="text" class="form-control bg-primary text-white" id="sale_{{$key}}_unit_name" name="sale[{{$key}}][unit_name]" value="{{$item->product->unit->unit_code}}" readonly/></td>
                                                        <td><input type="text" class="form-control price bg-primary text-white" id="sale_{{$key}}_price" name="sale[{{$key}}][price]" value="{{$item->net_unit_price}}" readonly/></td>
                                                        <td><input type="text" class="form-control bg-primary text-white" id="sale_{{$key}}_sale_qty" value="{{$item->qty}}" readonly/></td>
                                                        <td><input type="text" class="form-control bg-primary text-white" id="sale_{{$key}}_returned_qty" value="{{$item->return_qty}}" readonly/></td>
                                                        <td><input type="text" class="form-control return_qty" id="sale_{{$key}}_return_qty" data-price = "sale_{{$key}}_price" data-sale_qty = "sale_{{$key}}_sale_qty" data-returned_qty = "sale_{{$key}}_returned_qty" data-sub_total = "sale_{{$key}}_sub_total" name="sale[{{$key}}][return_qty]"/></td>
                                ...
                                <div class="form-group col-md-12 text-center pt-5">
                                    <button type="button" class="btn btn-primary btn-sm mr-3" id="save-btn" onclick="returnData()"><i class="fas fa-save"></i>{{__('Return')}}</button>
                                </div>
                            </div>
                        </form>
                    </div>
