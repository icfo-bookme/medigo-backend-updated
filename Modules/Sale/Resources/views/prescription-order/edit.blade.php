                                ...
                                <x-form.selectbox labelName="Customer" name="customer_id" col="col-md-3"  class="selectpicker">
                                    @if (!$customers->isEmpty())
                                        @foreach ($customers as $value)
                                            <option value="{{ $value->id }}">{{$value->id  == $sale->customer_id ? 'selected' : ''}}{{ $value->name.($value->mobile ? ' ('.$value->mobile.') ' : '') }}</option>
                                        @endforeach
                                    @endif
                                </x-form.selectbox>
                                            ...
                                            @foreach ($sale->sale_products as $key => $sale_product)
                                                <tr>
                                                    ...
                                                    <td class="text-center d-flex">
                                                        <a class="dropdown-item delete_item_data"  data-id="{{$sale_product->id}}" data-name="{{$sale_product->sale_id}}"><button type="button" class="btn btn-danger btn-sm  small-btn" style="padding: 1px 19px 19px 9px !important;font-size: 11px;border-radius: 6px;"><i class="fas fa-trash"></i></button></a>
...
@push('scripts')
    ...
    <script>
        ...
        $(document).on('click', '.delete_item_data', function () {
            let table = document.getElementById('product_table'); // Replace with your actual table ID or reference
            let id    = $(this).data('id');
            let name  = $(this).data('name');
            let row   =  $(this).closest('tr').index();
            // console.log(name);
            let url   = "{{ route('sale.delete.sale') }}";
            delete_sale_edit(id, url, table,row, name);
        });
    </script>
@endpush
