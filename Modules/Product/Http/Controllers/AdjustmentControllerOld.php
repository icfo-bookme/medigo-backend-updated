...
class AdjustmentControllerOld extends BaseController
{
                ...
                if ($request->has('products')) {
                    foreach ($request->products as $value) {
                        ...
                        if ($warehouse_product) {
                            ($value->action == '+') ? $warehouse_product->qty += $value->qty : $warehouse_product->qty -= $value->qty;
                            $warehouse_product->update();
                        } else {
                            WarehouseProduct::create([
                                'warehouse_id' => $request->warehouse_id,
                                'product_id' => $value->id,
                                'qty' => ($value->action == '+') ? $value->qty : -$value->qty
                            ]);
                        }
                    }
                    if (count($products) > 0) {
    ...
    {
        if ($request->ajax() && permission('adjustment-edit')) {
            DB::beginTransaction();
            try {
