            ...
            <div>
                <table style="margin-bottom:10px !important;">
                    <tr>
                        <td class="text-center">
                            ...
                            font-weight: bold;background: black;border-radius: 10px;width: 300px;color: white;text-align: center;padding:5px 0;}">STOCK REPORT</p>
                ...
                <table cellspacing="0" cellpadding="0" id="product_table">
                    ...
                    <tr>
                        ...
                        <td class="text-left font-weight-bolder">Product Name</td>
                        <td class="text-center font-weight-bolder">Code</td>
                        <td class="text-center font-weight-bolder">Company</td>
                        <td class="text-center font-weight-bolder">Category</td>
                        <td class="text-center font-weight-bolder">Stock Unit</td>
                        <td class="text-center font-weight-bolder">Stock Qty</td>
                        <td class="text-right font-weight-bolder">Price</td>
                        <td class="text-right font-weight-bolder">Stock Value</td>
                    </tr>
                    @php
                        $total_qty = 0;
                        $total = 0;
                    ...
                    @if (!$products->isEmpty())
                        @foreach ($products as $key => $value)
                            <tr>
                                ...
                                <td class="text-center"> {{$value->product->company ? $value->product->company->name : ''}} </td>
                                <td class="text-center">{{$value->product->category ? $value->product->category->name : ''}}   </td>
                                <td class="text-center"> {{$value->unit ? $value->unit->unit_name : ''}}</td>
                                <td class="text-center"> {{$value->qty}} </td>
                                <td class="text-right"> {{ number_format($value->price,2,'.',',') }} </td>
                                <td class="text-right"> {{ number_format(($value->price * $value->qty),2,'.',',') }}
                                    {{--                                @php --}}
                                    {{--                                    --}}
                                    {{--                                @endphp--}}
                                </td>
                    ...
                    @else
                        <tr>
                            <td colspan="9" class="text-center" style="color: red;font-weight:bold;">No Data Found</td>
                        </tr>
                    @endif
                    <tr>
                        <td style="font-weight:bold;" colspan="7">Total</td>
                        <td style="text-align: right !important;font-weight:bold;">{{ number_format($total_qty,2,'.',',') }}</td>
                        <td style="text-align: right !important;font-weight:bold;">{{ number_format($total,2,'.',',') }} /-TK</td>
                    </tr>
                    </tbody>
                </table>
            </div>
