                ...
                <div id="kt_datatable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                    <form id="contra-voucher-form" method="post">
                        ...
                        <div class="row">
                                ...
                                <input type="hidden" name="warehouse_id" id="warehouse_id" value="{{ $voucher[0]->warehouse_id }}">
                                @else
                                <x-form.selectbox labelName="Showroom" name="warehouse_id" required="required" col="col-md-3" class="selectpicker">
                                    @if (!$warehouses->isEmpty())
                                        @foreach ($warehouses as $value)
                                        <option value="{{ $value->id }}" {{ ($voucher[0]->warehouse_id == $value->id) ? 'selected' : '' }}>{{ $value->name }}</option>
                                        @endforeach
                                    @endif
                                </x-form.selectbox>
                                @endif
