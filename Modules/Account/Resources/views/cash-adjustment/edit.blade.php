                    ...
                    <form id="cash-adjustment-form" method="post">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <input type="hidden" name="warehouse_id" id="warehouse_id" value="{{ $voucher_data->warehouse_id }}">
                                @else
                                <x-form.selectbox labelName="Showroom" name="warehouse_id" required="required" col="col-md-6" class="selectpicker">
                                    @if (!$warehouses->isEmpty())
                                        @foreach ($warehouses as $value)
                                        <option value="{{ $value->id }}" {{ $voucher_data->warehouse_id == $value->id ? 'selected' : '' }}>{{ $value->name }}</option>
                                        @endforeach
                                    @endif
                                </x-form.selectbox>
                                @endif
