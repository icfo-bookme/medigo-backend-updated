            ...
            <div class="card card-custom gutter-b">
                <div class="card-header flex-wrap py-5">
                    ...
                    <div class="card-toolbar">
                        ...
                        <a href="{{ route('stock.return.list') }}" class="btn btn-warning btn-sm font-weight-bolder">
                            ...
                            <div class="invoice overflow-auto">
                                <div>
                                    ...
                                    <table>
                                        <tr>
                                            <td width="50%" class="text-left">
                                                ...
                                                <div class="m-0 date"><b>Exchange No.: {{ $sale->return_no }}</b></div>
                                    ...
                                    <table border="0" cellspacing="0" cellpadding="0">
                                        ...
                                        <tbody>
                                        @if (!$sale->exchange_products->isEmpty())
                                            @foreach ($sale->exchange_products as $key => $item)
                                                @php
                                                    $unit_name = '';
                                                    if($item->unit_id)
                                                    {
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-left">
                                                        {{ $item->product->name . ' - (' .$item->product_variant->item_code. ') - ['
                                                                .$item->product->brand->name . ']' }}
                                                    </td>
                                                    <td class="text-left">{{ $unit_name }}</td>
                                                    <td class="text-center qty">{{ $item->old_exchange_qty }}</td>
                                                    <td class="text-center price">{{ number_format($item->old_price,2) }}
                                                        @php
                                                            $g_t += $item->old_price
                                                        @endphp
                                                    </td>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                @if (config('settings.currency_position') == 2)
                                                    {{ number_format($g_t,2) }} {{ config('settings.currency_symbol') }}
                                                @else
                                                    {{ config('settings.currency_symbol') }} {{ number_format($g_t,2) }}
                                                @endif
                                            </td>
                                        </tr>
                                        </tfoot>
                                            ...
                                            <td class="text-center">
                                                <div class="font-size-10" style="width:250px;float:right;">
                                                    <p style="margin:35px 0 0 0;padding:0;"><b
                                                            class="text-uppercase">{{ $sale->created_by }}</b><br> {{ date('d-M-Y h:i:s A',strtotime($sale->created_at)) }}</p>
                                                    <p class="dashed-border"></p>
